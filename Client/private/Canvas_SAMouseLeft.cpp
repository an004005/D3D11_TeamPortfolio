...
// Mouse_Light : 0.6f ~ 0.7f 사이 로 알파값 조절하기
...
{
	if (m_bInput == false) return;

	dynamic_cast<CSA_AxisUI*>(Find_ChildUI(L"Arrow"))->Set_Move();	// 마우스 움직임 멈춰!

	IM_LOG("%f", m_fMoveDistance);
	...
	if (100.0f > fCircle_Size.x)
	{
		...
		fAlpha -= _float(TimeDelta) * 0.005f;
