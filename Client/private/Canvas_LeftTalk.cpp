...
void CCanvas_LeftTalk::Late_Tick(_double TimeDelta)
{
	CCanvas::Late_Tick(TimeDelta);

}

HRESULT CCanvas_LeftTalk::Render()
{
...
void CCanvas_LeftTalk::Imgui_RenderProperty()
{
	CCanvas::Imgui_RenderProperty();
}

void CCanvas_LeftTalk::SaveToJson(Json& json)
{
	CCanvas::SaveToJson(json);
}

void CCanvas_LeftTalk::LoadFromJson(const Json & json)
{
	CCanvas::LoadFromJson(json);

}

void CCanvas_LeftTalk::Set_Talk(const size_t iIndex)
{
	// x : [0] 가만히 [1] 공격중 [2] 웃음
	...
	dynamic_cast<CTalk_FaceUI*>(Find_ChildUI(L"Talk_Face"))->Set_Face(m_vecTalkInfo[iIndex].fFace, m_vecTalkInfo[iIndex].wsTalk0, m_vecTalkInfo[iIndex].wsTalk1);
	Find_ChildUI(L"Talk_Icon")->SetVisible(true);
}

void CCanvas_LeftTalk::TalkInfo_Initialize()
{
	TALKINFO	tTalkInfo;
	...
	m_vecTalkInfo.push_back(tTalkInfo);
