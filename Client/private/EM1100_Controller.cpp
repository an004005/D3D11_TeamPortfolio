...
void CEM1100_Controller::Tick_Near(_double TimeDelta)
{
	...
	//정면
	if (m_pCastedOwner->IsTargetFront(45.f) && m_bAttack == true)
	{
...
void CEM1100_Controller::Tick_Far(_double TimeDelta)
{
	...
	EMoveAxis eMoveAxis = EMoveAxis::AXIS_END;
	eMoveAxis = CMathUtils::RandomUInt(1) == 0 ? EMoveAxis::WEST : EMoveAxis::EAST;

	_bool IsFront = m_pCastedOwner->IsTargetFront(60.f);
