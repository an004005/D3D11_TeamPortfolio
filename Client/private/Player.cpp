...
//	m_pTransformCom->Set_State(CTransform::STATE_TRANSLATION, XMVectorSet(-5.f, 2.f, 5.f, 1.f));
	...
	CPlayerInfoManager::GetInstance()->ReleaseTeleportDissolve();
...
{
	if ((m_pModel->GetPlayAnimation()->GetName() == "AS_ch0100_051_AL_dodge_F_start" ||
...
HRESULT CPlayer::SetUp_BrainFieldProductionStateMachine()
{
	m_pBrainFieldProductStateMachine =
		...
		.OnExit([&]() {})
			...
			.Predicator([&]()->_bool { return (0.8f <= m_pModel->Find_Animation("AS_ch0100_BrainField_start")->GetPlayRatio()); })
			.Priority(0)
		...
		.OnStart([&]() 
		{
			...
			auto pCamAnim = CGameInstance::GetInstance()->GetCamAnim("BrainField_02");
			m_pPlayer_AnimCam->StartCamAnim_Return_Update(pCamAnim, m_pPlayerCam, m_pTransformCom, m_fCameraLerpTime_In, m_fCameraLerpTime_Out);
...
HRESULT CPlayer::SetUp_EffectEvent()
{
	...
	*/
	...
	// ∫Í∑π¿Œ« µÂ ∞¸∑√
	m_pModel->Add_EventCaller("BrainField_Ring_Effect", [&]() {
	...
	m_pModel->Add_EventCaller("BrainField_Gear_Effect", [&]() {
	...
	m_pModel->Add_EventCaller("BrainField_CableSpark_Effect_A", [&]() {
		_matrix EffectMatrix = XMMatrixScaling(0.3f, 0.3f, 0.3f) * XMMatrixRotationX(-90.f);
		CVFX_Manager::GetInstance()->GetEffect(EFFECT::EF_SAS, L"Cable_Connected_A")->Start_AttachPivot(this, EffectMatrix, "BackHair", true, true);
	});

	m_pModel->Add_EventCaller("BrainField_CableSpark_Effect_B", [&]() {
		_matrix EffectMatrix = XMMatrixScaling(0.3f, 0.3f, 0.3f) * XMMatrixRotationX(-90.f);
		CVFX_Manager::GetInstance()->GetEffect(EFFECT::EF_SAS, L"Cable_Connected_B")->Start_AttachPivot(this, EffectMatrix, "BackHair", true, true);
	});

	m_pModel->Add_EventCaller("BrainField_Cable_Particle", [&]() {
		CVFX_Manager::GetInstance()->GetParticle(PARTICLE::PS_SAS, L"Sas_Dead_Particles")->Start_Attach(this, "BackHair", false);
	});

	m_pModel->Add_EventCaller("BrainField_RedTextParticle_Head", [&]() {
		CVFX_Manager::GetInstance()->GetParticle(PARTICLE::PS_SAS, L"BrainField_RedText_Particle")->Start_Attach(this, "BackHair", false);
	});

	m_pModel->Add_EventCaller("BrainField_RedTextParticle_Explode", [&]() {
		CVFX_Manager::GetInstance()->GetParticle(PARTICLE::PS_SAS, L"BrainField_Sphere_Red_Text")->Start_Attach(this, "Waist", false);
	});
...
HRESULT CPlayer::SetUp_Sound()
{
	...
	m_SoundStore.CloneSound("fx_impact_player_hit");	// æ‡««∞›
	m_SoundStore.CloneSound("fx_plyr_hit_metalic");		// ¡ﬂ««∞›
	m_SoundStore.CloneSound("fx_impact_player_hit_3");	// ∞≠««∞›
...
HRESULT CPlayer::SetUp_AttackDesc()
{
	...
	m_mapCollisionEvent.emplace("ATK_A3", [this]()
	{
		...
		m_AttackDesc.iDamage = static_cast<_uint>(CPlayerInfoManager::GetInstance()->Get_PlayerStat().m_fBaseAttackDamage * 1.3f);
		m_AttackDesc.pCauser = this;
		m_AttackDesc.vHitFrom = m_pTransformCom->Get_State(CTransform::STATE_TRANSLATION);
	});
	m_mapCollisionEvent.emplace("ATK_A4", [this]()
	{
		...
		m_AttackDesc.iDamage = static_cast<_uint>(CPlayerInfoManager::GetInstance()->Get_PlayerStat().m_fBaseAttackDamage * 0.7f);
		m_AttackDesc.pCauser = this;
		m_AttackDesc.vHitFrom = m_pTransformCom->Get_State(CTransform::STATE_TRANSLATION);
	});
	m_mapCollisionEvent.emplace("ATK_A5", [this]()
	{
		...
		m_AttackDesc.iDamage = static_cast<_uint>(CPlayerInfoManager::GetInstance()->Get_PlayerStat().m_fBaseAttackDamage * 2.5f);
		m_AttackDesc.pCauser = this;
		m_AttackDesc.vHitFrom = m_pTransformCom->Get_State(CTransform::STATE_TRANSLATION);
	});
	...
	m_mapCollisionEvent.emplace("ATTACK_NONCHARGE", [this]()
	{
		...
		m_AttackDesc.iDamage = static_cast<_uint>(CPlayerInfoManager::GetInstance()->Get_PlayerStat().m_fBaseAttackDamage * 1.5f);
		m_AttackDesc.pCauser = this;
		m_AttackDesc.vHitFrom = m_pTransformCom->Get_State(CTransform::STATE_TRANSLATION);
	});
	...
	m_mapCollisionEvent.emplace("CHARGE_ATTACK_02", [this]()
	{
		...
		m_AttackDesc.iDamage = static_cast<_uint>(CPlayerInfoManager::GetInstance()->Get_PlayerStat().m_fBaseAttackDamage * 2.f);
		m_AttackDesc.pCauser = this;
		m_AttackDesc.vHitFrom = m_pTransformCom->Get_State(CTransform::STATE_TRANSLATION);
	});
	...
	m_mapCollisionEvent.emplace("ATTACK_UPPER_START", [this]()
	{
		...
		m_AttackDesc.iDamage = static_cast<_uint>(CPlayerInfoManager::GetInstance()->Get_PlayerStat().m_fBaseAttackDamage * 1.1f);
		m_AttackDesc.pCauser = this;
		m_AttackDesc.vHitFrom = m_pTransformCom->Get_State(CTransform::STATE_TRANSLATION);
	});
	...
	m_mapCollisionEvent.emplace("AS_ch0100_214_AL_sas_dash_end_Telepo", [this]()
	{
		...
		m_AttackDesc.iDamage = static_cast<_uint>(CPlayerInfoManager::GetInstance()->Get_PlayerStat().m_fBaseAttackDamage * 1.2f);
		m_AttackDesc.pCauser = this;
		m_AttackDesc.vHitFrom = m_pTransformCom->Get_State(CTransform::STATE_TRANSLATION);
	});
...
HRESULT CPlayer::SetUp_TrainStateMachine()
{
	...
	m_pTrainStateMachine_Left = 
		...
		.OnStart([&]() 
		{
			...
			m_pASM->ClearAnimSocket("Kinetic_Special_AnimSocket");
...
HRESULT CPlayer::SetUp_TelephonePoleStateMachine()
{
	...
	m_pTelephonePoleStateMachine_Left =
		...
		.OnStart([&]() 
		{
			ActiveSpecialUI(SPECIAL_TELEPHONEPOLE);
...
HRESULT CPlayer::SetUp_ContainerStateMachine()
{
	...
	m_pContainerStateMachine =
		...
		.OnStart([&]() 
		{
			ActiveSpecialUI(SPECAIL_CONTAINER , false);
		})
		.Tick([&](double fTimeDelta)
		{
...
void CPlayer::NetualChecker(_double TimeDelta)
{
	...
	if (m_bOnBattle && !m_bBrainField_Prod && !CPlayerInfoManager::GetInstance()->Get_PlayerStat().bBrainField)	// ?ÑÌà¨ Ï§??åÌã∞???ùÏÑ±
	{
...
void CPlayer::ActiveSpecialUI(ESpecialType eType, _bool bAdditional)
{
	...
	case SPECIAL_TELEPHONEPOLE:
		...
		if (CGameInstance::GetInstance()->Check_ObjectAlive(m_pSpecialUI))
		{
			static_cast<CCanvas_SAMouseLeft*>(m_pSpecialUI)->Set_Intput();
		}
	...
	case SPECIAL_HBEAM_BUNDLE:
		...
		if (CGameInstance::GetInstance()->Check_ObjectAlive(m_pSpecialUI))
		{
			static_cast<CCanvas_SARebar*>(m_pSpecialUI)->Set_Intput();
		}
	...
	case SPECIAL_IRONBARS:
		...
		if (bAdditional)
		{
			if (CGameInstance::GetInstance()->Check_ObjectAlive(m_pSpecialUI))
			{
				static_cast<CCanvas_SAGragting_Go*>(m_pSpecialUI)->Set_Input();
			}
		}
		else
		{
			if (CGameInstance::GetInstance()->Check_ObjectAlive(m_pSpecialUI))
			{
				static_cast<CCanvas_SAGragting_S*>(m_pSpecialUI)->Set_Intput();
			}
		}
	...
	case SPECAIL_CONTAINER:
		...
		if (bAdditional)
		{
			if (CGameInstance::GetInstance()->Check_ObjectAlive(m_pSpecialUI))
			{
				static_cast<CCanvas_SAContainer_Down*>(m_pSpecialUI)->Set_Input();
			}
		}
		else
		{
