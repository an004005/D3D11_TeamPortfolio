		...
		m_pASM_Base = CASMBuilder()
			...
			.AddState("Idle")
				...
				/*.AddTransition("Idle to JumpStart", "JumpStart")
					.Predicator([&]()->_bool {return m_bAir; })
					.Duration(0.2f)*/
			/*
			.AddTransition("Idle to Attack", "Attack")
			.Predicator([&]()->_bool {return m_bAttackL || m_bAttackR; })*/
			...
			//		.Predicator([&]()->_bool {return !m_bAir && m_bLanding; })	// End를 위한 값 받기
			//		.Duration(0.2f)

			//.AddState("JumpEnd")
			//	.SetAnimation(*m_pModel->Find_Animation("AS_em0200_136_AL_dodge_B_stop"))

			//	.AddTransition("JumpEnd to Idle", "Idle")
			//		.Predicator([&]()->_bool {return !m_bLanding && m_bIdle; })
			//		.Duration(0.2f)

			/*.AddState("AttackL")
				.SetAnimation(*m_pModel->Find_Animation("AS_em0200_202_AL_atk_a2"))

				.AddTransition("AttackL to Idle", "Idle")
					.Predicator([&]()->_bool {return !m_bAttackL; })
					.Duration(0.2f)

			.AddState("AttackR")
				.SetAnimation(*m_pModel->Find_Animation("AS_em0200_203_AL_atk_a3"))

				.AddTransition("AttackR to Idle", "Idle")
					.Predicator([&]()->_bool {return !m_bAttackR; })
					.Duration(0.2f)*/
...
void CFL_AnimInstance::UpdateTargetState(_double TimeDelta)
{
	...
	m_bIdle = pFlowerLeg->IsIdle();
	m_bWalk = pFlowerLeg->IsWalk();
	m_bAttackL = pFlowerLeg->IsAttackL();
	m_bAttackR = pFlowerLeg->IsAttackR();
	m_bAir = pFlowerLeg->IsAir();
	m_bLanding = pFlowerLeg->IsLanding();
}
