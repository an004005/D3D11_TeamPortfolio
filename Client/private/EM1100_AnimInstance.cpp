...
HRESULT CEM1100_AnimInstance::Initialize(CModel * pModel, CGameObject * pGameObject)
{
...
void CEM1100_AnimInstance::UpdateTargetState(_double TimeDelta)
{
	CEM1100* pEM1100 = static_cast<CEM1100*>(m_pTargetObject);
...
CEM1100_AnimInstance * CEM1100_AnimInstance::Create(CModel * pModel, CGameObject * pGameObject)
{
	CEM1100_AnimInstance*		pInstance = new CEM1100_AnimInstance();

	if (FAILED(pInstance->Initialize(pModel, pGameObject)))
	{
		MSG_BOX("Failed to Created : CEM1100_AnimInstance");
		Safe_Release(pInstance);
	}
	return pInstance;
}

void CEM1100_AnimInstance::Free()
{
	__super::Free();
