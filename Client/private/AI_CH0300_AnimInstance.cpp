...
#pragma region HIT_LIGHT
		...
		.AddState("HIT_LIGHT_FRONT")
			.SetAnimation(*m_pModel->Find_Animation("AS_ch0300_401_AL_damage_l_F"))
			.StartEvent([&]() 
			{ 
				Reset_HitCheck();
			})
		...
		.AddState("HIT_LIGHT_BACK")
			.SetAnimation(*m_pModel->Find_Animation("AS_ch0300_402_AL_damage_l_B"))
			.StartEvent([&]() 
			{ 
				Reset_HitCheck();
			})
...
#pragma region HIT_MIDDLE
		...
		.AddState("HIT_MIDDLE_FRONT")
			.SetAnimation(*m_pModel->Find_Animation("AS_ch0300_411_AL_damage_h_F"))
			.StartEvent([&]() 
			{ 
				Reset_HitCheck();
			})
		...
		.AddState("HIT_MIDDLE_BACK")
			.SetAnimation(*m_pModel->Find_Animation("AS_ch0300_412_AL_damage_h_B"))
			.StartEvent([&]() 
			{ 
				Reset_HitCheck();
			})
		...
		.AddState("HIT_MIDDLE_LEFT")
			.SetAnimation(*m_pModel->Find_Animation("AS_ch0300_413_AL_damage_h_L"))
			.StartEvent([&]() 
			{ 
				Reset_HitCheck();
			})
		...
		.AddState("HIT_MIDDLE_RIGHT")
			.SetAnimation(*m_pModel->Find_Animation("AS_ch0300_414_AL_damage_h_R"))
			.StartEvent([&]() 
			{ 
				Reset_HitCheck();
			})
...
#pragma region HIT_KNUCKBACK
		.AddState("HIT_KNUCKBACK_START")
			.SetAnimation(*m_pModel->Find_Animation("AS_ch0300_455_AL_rise_start"))
				.AddTransition("HIT_KNUCKBACK_START to HIT_BLOW_LANDING", "HIT_BLOW_LANDING")
				.Duration(0.f)
				.Priority(0)

		.AddState("HIT_BLOW_LANDING")
			.SetAnimation(*m_pModel->Find_Animation("AS_ch0300_433_AL_blow_landing_F"))
				.AddTransition("HIT_FALLDOWN to HIT_DOWN", "HIT_DOWN")
				.Duration(0.f)
				.Priority(0)

		.AddState("HIT_DOWN")
			.SetAnimation(*m_pModel->Find_Animation("AS_ch0300_434_AL_down_F"))
				.AddTransition("HIT_DOWN to HIT_WAKEUP", "HIT_WAKEUP")
				.Duration(0.f)
				.Priority(0)

		.AddState("HIT_WAKEUP")
			.SetAnimation(*m_pModel->Find_Animation("AS_ch0300_435_AL_wakeup_F"))
			.StartEvent([&]() 
			{ 
				Reset_HitCheck();
			})
...
#pragma region HIT_AIRBORNE
	...
	.AddState("HIT_AIRBORNE")
		...
		.StartEvent([&]() 
		{ 
			static_cast<CAI_CH0300*>(m_pTargetObject)->Set_Jump(10.f);
		})
	...
	.AddState("HIT_FALLDOWN")
		.SetAnimation(*m_pModel->Find_Animation("AS_ch0300_457_AL_fall_start"))
			.AddTransition("HIT_FALLDOWN to HIT_BLOW_LANDING", "HIT_BLOW_LANDING")
			.Predicator([&]()->_bool {return static_cast<CAI_CH0300*>(m_pTargetObject)->Get_OnFloor(); })
			.Duration(0.f)
			.Priority(0)
...
#pragma region DODGE
		...
		.AddState("DODGE_START")
			.SetAnimation(*m_pModel->Find_Animation("AS_ch0300_035_AL_dodge_B_start"))
				.AddTransition("DODGE_START to DODGE_END", "DODGE_END")
				.Duration(0.f)
				.Priority(0)

		.AddState("DODGE_END")
			.SetAnimation(*m_pModel->Find_Animation("AS_ch0300_036_AL_dodge_B_stop"))
...
#pragma region JUMP
	...
	.AddState("JUMP_START")
		.SetAnimation(*m_pModel->Find_Animation("AS_ch0300_023_AL_jump_start"))
		.AddTransition("JUMP_START to JUMP_RISE", "JUMP_RISE")
		.Duration(0.1f)
		.Priority(0)

	.AddState("JUMP_RISE")
		.SetAnimation(*m_pModel->Find_Animation("AS_ch0300_024_AL_jump_rise"))
		.StartEvent([&]() { static_cast<CAI_CH0300*>(m_pTargetObject)->Set_Jump(10.f); })
		...
		{
			_bool bResult =
				static_cast<CAI_CH0300*>(m_pTargetObject)->Get_OnFloor() &&
				(0.2f <= m_pModel->Find_Animation("AS_ch0300_024_AL_jump_rise")->GetPlayRatio());
	...
	.AddState("JUMP_FALL")
		.SetAnimation(*m_pModel->Find_Animation("AS_ch0300_021_AL_jump_fall"))
	...
	.AddState("JUMP_LANDING")
		.SetAnimation(*m_pModel->Find_Animation("AS_ch0300_022_AL_jump_landing"))
		.StartEvent([&]() 
		{ 
			static_cast<CAI_CH0300*>(m_pTargetObject)->Reset_Jump();
			m_bJump = false;
		})
...
void CAI_CH0300_AnimInstance::SpairAnimationManager(_bool bBattle)
{
	if (bBattle)
	{
		...
		m_pASM_Base->SetSpairAnim("JUMP_START", m_pModel->Find_Animation("AS_ch0300_123_AL_jump_start"));
		m_pASM_Base->SetSpairAnim("JUMP_RISE", m_pModel->Find_Animation("AS_ch0300_124_AL_jump_rise"));
		m_pASM_Base->SetSpairAnim("JUMP_FALL", m_pModel->Find_Animation("AS_ch0300_121_AL_jump_fall"));
...
void CAI_CH0300_AnimInstance::Reset_HitCheck()
{
	static_cast<CAI_CH0300*>(m_pTargetObject)->Reset_Hit();
	static_cast<CAI_CH0300*>(m_pTargetObject)->Reset_DamageDesc();
	m_bHit = false;
	ZeroMemory(&m_tDamageDesc, sizeof(DAMAGE_DESC));
}
