...
void CCanvas_Item::Imgui_RenderProperty()
{
	...
	if (ImGui::Button("su3"))
	{
		CItem_Manager::GetInstance()->Set_ItemCount(L"SAS 보급수", 3);
	}

	if (ImGui::Button("rlrl"))
	{
		CItem_Manager::GetInstance()->Set_ItemCount(L"브레인 필드 기어", 1);
	}

	if (ImGui::Button("big"))
	{
		CItem_Manager::GetInstance()->Set_ItemCount(L"전체: 회복(대) 젤리", 5);
	}
}
