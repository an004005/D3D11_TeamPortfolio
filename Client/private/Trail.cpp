...
#include "VIBuffer_Instancing_Trail.h"
#include "MathUtils.h"
#include "GameUtils.h"
...
HRESULT CTrail::Initialize(void* pArg)
{
	...
	if (FAILED(__super::Add_Component(LEVEL_STATIC, TEXT("Prototype_Component_Renderer"), TEXT("Com_Renderer"),
		...
		return E_FAIL;
	if (FAILED(__super::Add_Component(LEVEL_STATIC, TEXT("Prototype_Component_Shader_Trail_VFX"), TEXT("Shader"),
		(CComponent**)&m_pShaderCom)))
		return E_FAIL;

	m_pBuffer = CVIBuffer_Instancing_Trail::Create(m_pDevice, m_pContext, 300);
	...
	if (pArg)
	{
		...
		if (FAILED(__super::Add_Component(LEVEL_STATIC, texProtoTag.c_str(), TEXT("Texture"),
			(CComponent**)&m_pTex)))
			return E_FAIL;
	}
	else
	{
		if (FAILED(__super::Add_Component(LEVEL_STATIC, TEXT("../Bin/Resources/Textures/VFX/AlphaGradient/ring_glow.dds"), TEXT("Texture"),
			(CComponent**)&m_pTex)))
			return E_FAIL;
...
void CTrail::Tick(_double TimeDelta)
{
	...
	// m_pTransformCom->Turn(m_pTransformCom->Get_State(CTransform::STATE_UP), TimeDelta);
...
void CTrail::SaveToJson(Json& json)
{
	json["Life"] = m_fLife;
	json["Width"] = m_fWidth;
...
void CTrail::Imgui_RenderProperty()
{
