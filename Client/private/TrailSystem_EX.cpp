#include "stdafx.h"
#include "../public/TrailSystem_EX.h"
...
CTrailSystem_EX::CTrailSystem_EX(ID3D11Device* pDevice, ID3D11DeviceContext* pContext)
	: CGameObject(pDevice, pContext)
{
}

CTrailSystem_EX::CTrailSystem_EX(const CTrailSystem_EX& rhs)
	: CGameObject(rhs)
{
}

HRESULT CTrailSystem_EX::Initialize(void* pArg)
{
...
void CTrailSystem_EX::Tick(_double TimeDelta)
{
...
HRESULT CTrailSystem_EX::Render()
{
	...
	m_pShaderCom->Set_MatrixArray("g_TrailData", (_float4x4*)pData->data(), static_cast<_uint>(pData->size()));
	m_pShaderCom->Set_RawValue("g_fLife", &m_fLife, sizeof(_float));
	m_pShaderCom->Set_RawValue("g_fWidth", &m_fWidth, sizeof(_float));
	m_pShaderCom->Set_RawValue("g_Color", &m_vColor, sizeof(_float4));
	m_pTex->Bind_ShaderResource(m_pShaderCom, "g_AlphaTex");
...
void CTrailSystem_EX::SaveToJson(Json& json)
{
	json["Life"] = m_fLife;
	json["Width"] = m_fWidth;
...
void CTrailSystem_EX::Imgui_RenderProperty()
{
...
HRESULT CTrailSystem_EX::SetUp_ShaderResources()
{
...
void CTrailSystem_EX::ClearBuffer()
{
	m_pBuffer->ClearBuffer();
	m_vPrePoses.clear();
}

CTrailSystem_EX* CTrailSystem_EX::Create(ID3D11Device* pDevice, ID3D11DeviceContext* pContext)
{
	CTrailSystem_EX* pInstance = new CTrailSystem_EX(pDevice, pContext);

	if (FAILED(pInstance->Initialize_Prototype()))
	{
		MSG_BOX("Failed to Created : CTrailSystem_EX");
		Safe_Release(pInstance);
	}
	return pInstance;
}

CGameObject* CTrailSystem_EX::Clone(void* pArg)
{
	CTrailSystem_EX* pInstance = new CTrailSystem_EX(*this);

	if (FAILED(pInstance->Initialize(pArg)))
	{
		MSG_BOX("Failed to Cloned : CTrailSystem_EX");
		Safe_Release(pInstance);
	}
	return pInstance;
}

void CTrailSystem_EX::Free()
{
	CGameObject::Free();
